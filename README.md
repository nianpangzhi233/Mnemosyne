...
Web Dashboard（纯 HTML + JS，零依赖）：

```bash
# 直接打开
start scripts/dashboard/web/index.html
# 或启动 REST API 后访问（实时数据）
start-v8-api.cmd
# 然后修改 index.html 中的 API_BASE 为 http://127.0.0.1:8979/api/v8
```
...
- [x] Web Dashboard（纯 HTML + JS，部署到 GitHub Pages）
- [ ] LLM 驱动的自动 Evidence 生成（保持人工审核）
...
Web Dashboard (pure HTML + JS, zero dependencies):

```bash
# Open directly
start scripts/dashboard/web/index.html
# Or start REST API for live data
start-v8-api.cmd
# Then set API_BASE in index.html to http://127.0.0.1:8979/api/v8
```
...
- [x] Web Dashboard (pure HTML + JS, deployed to GitHub Pages)
- [ ] LLM-driven automatic Evidence generation (with human review)
